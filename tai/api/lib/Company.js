...
class Company {
    ...
    async del_companys(_from, _companyid) {
        try {
            ...
            if (companyexistence == true) {
                try {
                    const del_companys_stat = await MyContract.methods.DelCompany(_companyid).send({
                        from: _from,
                        gas: config.gas,
                        gasPrice: config.gasPrice
                    });
                    return {
                        code: 200,
                        message: '删除公司成功',
                        detail: '交易hash:' + del_companys_stat.transactionHash
                    };
                } catch (err) {
                    return {
                        code: 400,
                        'message': 'eth拒绝交易',
                        'detail': '发起者不是该公司的属主:' + err
                    };
                }
            } else {
                return {
    ...
    async update_companys(_from, companyid, email, remark, stat) {
        try {
            const companyexistence = await MyContract.methods.ShowCompanyidExist(companyid).call({
                from: _from
            });
            if (companyexistence == true) {
                try {
                    const update_companys_stat = await MyContract.methods.UpdateCompany(companyid, email, remark, stat).send({
                        from: _from,
                        gas: config.gas,
                        gasPrice: config.gasPrice
                    });
                    return {
                        code: 200,
                        message: '更新公司成功',
                        detail: '交易hash:' + update_companys_stat.transactionHash
                    };
                } catch (err) {
                    return {
                        code: 400,
    ...
    async get_companys(_from, companyid) {
        try {
            ...
            if (companyexistence == true) {
                try {
                    const get_companys_stat = await MyContract.methods.ShowCompany(companyid).call({
                        from: _from
                    }, );
                    return {
                        code: 200,
                        message: '查询' + companyid + '成功',
                        detail: get_companys_stat
                    };
                } catch (err) {
                    return {
                        code: 400,
    ...
    async get_all_companys(_from) {
        var Arr = [];
        const get_companys_stat = await MyContract.methods.CompanySum().call({
            from: _from
        }, );
        for (var i = 1; i < get_companys_stat; i++) {

            try {
                const temp = await MyContract.methods.ShowCompany(i).call({
                    from: _from
                }, );
                console.log(temp)
                Arr.push(temp)
