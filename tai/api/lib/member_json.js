...
const MyContract = new web3.eth.Contract(config.abi, contractaddress);
    (async()=>{  
        ...
        for(j = 1; j <=sum; j++) {
            const checkmember_stat=await MyContract.methods.isMember(j).call({from: config.account});
            if (checkmember_stat == true){
                const getcompany_stat=await MyContract.methods.ShowCompany(j).call({from: config.account});
        
                array.push(getcompany_stat[4]);
        ...
        fs.writeFile('lib/member.json',person,function(err){
            if(err){
                console.error(err);
            }
            console.log('----------创建enode列表成功-------------');
        })
